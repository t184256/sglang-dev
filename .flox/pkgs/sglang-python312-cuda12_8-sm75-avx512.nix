# SGLang 0.5.9 for NVIDIA Turing (SM75: T4, RTX 2080 Ti) — AVX-512
# CUDA 12.8 — Requires NVIDIA driver 550+
# Custom PyTorch built from source (SM75 + AVX-512)
{ pkgs ? import <nixpkgs> {} }:
let
  ...
  smCapability = "7.5";
  variantName = "sglang-python312-cuda12_8-sm75-avx512";
...
in
  sglang.overrideAttrs (oldAttrs: {
    ...
    meta = oldAttrs.meta // {
      description = "SGLang 0.5.9 for NVIDIA T4/RTX 2080 Ti (SM75) [CUDA 12.8, custom PyTorch AVX-512]";
      platforms = [ platform ];
    };
  })
