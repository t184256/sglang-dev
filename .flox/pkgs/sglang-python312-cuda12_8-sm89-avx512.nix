# SGLang 0.5.9 for NVIDIA Ada Lovelace (SM89: RTX 4090, L4, L40) — AVX-512
# CUDA 12.8 — Requires NVIDIA driver 550+
# Custom PyTorch built from source (SM89 + AVX-512)
{ pkgs ? import <nixpkgs> {} }:
let
  ...
  smCapability = "8.9";
  variantName = "sglang-python312-cuda12_8-sm89-avx512";
...
in
  sglang.overrideAttrs (oldAttrs: {
    ...
    meta = oldAttrs.meta // {
      description = "SGLang 0.5.9 for NVIDIA RTX 4090/L4/L40 (SM89) [CUDA 12.8, custom PyTorch AVX-512]";
      platforms = [ platform ];
    };
  })
