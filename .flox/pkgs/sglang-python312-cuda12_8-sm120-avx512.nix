# SGLang 0.5.9 for NVIDIA Blackwell (SM120: RTX 5090, RTX PRO 6000) — AVX-512
# CUDA 12.8 — Requires NVIDIA driver 550+
# Custom PyTorch built from source (SM120 + AVX-512)
{ pkgs ? import <nixpkgs> {} }:
let
  ...
  smCapability = "12.0";
  variantName = "sglang-python312-cuda12_8-sm120-avx512";
...
in
  sglang.overrideAttrs (oldAttrs: {
    ...
    meta = oldAttrs.meta // {
      description = "SGLang 0.5.9 for NVIDIA RTX 5090/RTX PRO 6000 (SM120) [CUDA 12.8, custom PyTorch AVX-512]";
      platforms = [ platform ];
    };
  })
