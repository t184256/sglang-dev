# SGLang 0.5.9 for NVIDIA Ampere DC (SM80: A100, A30) — AVX-512
# CUDA 12.8 — Requires NVIDIA driver 550+
# Custom PyTorch built from source (SM80 + AVX-512)
{ pkgs ? import <nixpkgs> {} }:
let
  ...
  smCapability = "8.0";
  variantName = "sglang-python312-cuda12_8-sm80-avx512";
...
in
  sglang.overrideAttrs (oldAttrs: {
    ...
    meta = oldAttrs.meta // {
      description = "SGLang 0.5.9 for NVIDIA A100/A30 (SM80) [CUDA 12.8, custom PyTorch AVX-512]";
      platforms = [ platform ];
    };
  })
