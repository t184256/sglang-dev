# SGLang 0.5.9 — all GPU architectures (SM61–SM120) — AVX2
# CUDA 12.8 — Requires NVIDIA driver 550+
# Custom PyTorch built from source (all SMs + AVX2, USE_CUDNN=0)
{ pkgs ? import <nixpkgs> {} }:
let
...
in
  sglang.overrideAttrs (oldAttrs: {
    ...
    meta = oldAttrs.meta // {
      description = "SGLang 0.5.9 for all NVIDIA GPUs SM61-SM120 [CUDA 12.8, custom PyTorch AVX2, no cuDNN]";
      platforms = [ platform ];
    };
  })
