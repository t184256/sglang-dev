# SGLang 0.5.9 for NVIDIA Pascal (SM61: P40, GTX 1080 Ti) — AVX-512
# CUDA 12.8 — Requires NVIDIA driver 550+
# Custom PyTorch built from source (SM61 + AVX-512, USE_CUDNN=0)
{ pkgs ? import <nixpkgs> {} }:
let
  ...
  smCapability = "6.1";
  variantName = "sglang-python312-cuda12_8-sm61-avx512";
...
in
  sglang.overrideAttrs (oldAttrs: {
    ...
    meta = oldAttrs.meta // {
      description = "SGLang 0.5.9 for NVIDIA P40/GTX 1080 Ti (SM61) [CUDA 12.8, custom PyTorch AVX-512, no cuDNN]";
      platforms = [ platform ];
    };
  })
