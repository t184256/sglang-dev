# SGLang 0.5.9 for NVIDIA Blackwell DC (SM100: B100, B200, GB200) — AVX-512
# CUDA 12.8 — Requires NVIDIA driver 550+
# Custom PyTorch built from source (SM100 + AVX-512)
{ pkgs ? import <nixpkgs> {} }:
let
  ...
  smCapability = "10.0";
  variantName = "sglang-python312-cuda12_8-sm100-avx512";
...
in
  sglang.overrideAttrs (oldAttrs: {
    ...
    meta = oldAttrs.meta // {
      description = "SGLang 0.5.9 for NVIDIA B100/B200/GB200 (SM100) [CUDA 12.8, custom PyTorch AVX-512]";
      platforms = [ platform ];
    };
  })
